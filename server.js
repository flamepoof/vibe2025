...
const app = express();
const port = 3000;
const telegram_token = '7751087406:AAFAKJ_xpOIstfj0azxHdfccMOqCJl2sCSw';
...
async function linkTelegramId(username, password, telegramId) {
  try {
    ...
    const [telegramRows] = await (await mysql.createConnection(dbConfig))
      .execute('SELECT * FROM users WHERE telegram_id = ?', [telegramId]);
    ...
    await (await mysql.createConnection(dbConfig))
      .execute('UPDATE users SET telegram_id = ? WHERE username = ?', [telegramId, username]);
...
bot.onText(/\/start/, async (msg) => {
  ...
  const onUsername = async (msg) => {
    username = msg.text.trim();
    const user = await getUserByUsername(username);
    
    if (!user) {
      bot.sendMessage(chatId, `Пользователь с логином "${username}" не найден. Зарегистрируйтесь на сайте.`);
      bot.removeListener('message', onUsername);
      return;
    }
    
    bot.sendMessage(chatId, 'Теперь введите ваш пароль:');
    bot.removeListener('message', onUsername);
    
    const onPassword = async (msg) => {
      const password = msg.text.trim();
      const result = await linkTelegramId(username, password, chatId.toString());
      bot.sendMessage(chatId, result.message);
      bot.removeListener('message', onPassword);
    };
    
    bot.once('message', onPassword);
  };
  
  bot.once('message', onUsername);
});
